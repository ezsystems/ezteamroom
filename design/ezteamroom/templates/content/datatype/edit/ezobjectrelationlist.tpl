...
{if $class_content.selection_type|ne( 0 )} {* If current selection mode is not 'browse'. *}
        ...
        <div class="buttonblock">
            ...
            <select name="{$attribute_base}_data_object_relation_list_{$attribute.id}[]">
                {section show=$attribute.contentclass_attribute.is_required|not}
                    <option value="no_relation" {section show=eq( $attribute.content.relation_list|count, 0 )} selected="selected"{/section}>{'No relation'|i18n( 'ezteamroom/edit' )}</option>
                {/section}
        ...
        {case match=2} {* radio buttons list *}
            ...
            {section show=$attribute.contentclass_attribute.is_required|not}
                ...
                {section show=eq( $attribute.content.relation_list|count, 0 )} checked="checked"{/section}>{'No relation'|i18n( 'ezteamroom/edit' )}<br />{/section}
            {section var=node loop=$nodesList}
        ...
        {if eq( count( $nodesList ), 0 )}
            ...
            {if is_set( $parentnode )}
                <p>{'Parent node'|i18n( 'ezteamroom/edit' )}: {node_view_gui content_node=$parentnode view=objectrelationlist} </p>
            {/if}
            <p>{'Allowed classes'|i18n( 'ezteamroom/edit' )}:</p>
            ...
            {else}
                 <ul>
                       <li>{'Any'|i18n( 'ezteamroom/edit' )}</li>
                 </ul>
            {/if}
            <p>{'There are no objects of allowed classes'|i18n( 'ezteamroom/edit' )}.</p>
        {/if}
        ...
        {section show = and( is_set( $class_content.default_placement.node_id ), ne( 0, $class_content.default_placement.node_id ), ne( '', $class_content.object_class ) )}
            ...
            {if and( is_set( $defaultNode ), $defaultNode.can_create )}
                <div id='{$attribute.id}_create_new_object' style="display:none;">
                     <p>{'Create new object with name'|i18n( 'ezteamroom/edit' )}:</p>
...
{else}  {* Standard mode is browsing *}
        ...
        <tr class="bglight">
            <th class="tight"><img src={'toggle-button-16x16.gif'|ezimage} alt="{'Invert selection.'|i18n( 'ezteamroom/edit' )}" onclick="ezjs_toggleCheckboxes( document.editform, '{$attribute_base}_selection[{$attribute.id}][]' ); return false;" title="{'Invert selection.'|i18n( 'ezteamroom/edit' )}" /></th>
            <th>{'Name'|i18n( 'ezteamroom/edit' )}</th>
            <th>{'Type'|i18n( 'ezteamroom/edit' )}</th>
            <th>{'Section'|i18n( 'ezteamroom/edit' )}</th>
            <th class="tight">{'Order'|i18n( 'ezteamroom/edit' )}</th>
        </tr>
        ...
        <fieldset>
        <legend>{'Edit <%object_name> [%object_class]'|i18n( 'ezteamroom/edit',, hash( '%object_name', $Relation:object.name, '%object_class', $Relation:object.class_name ) )|wash}</legend>
        ...
        {section-else}
        <p>{'There are no related objects.'|i18n( 'ezteamroom/edit' )}</p>
        {/section}

        {section show=$attribute.content.relation_list}
        <input class="button" type="submit" name="CustomActionButton[{$attribute.id}_remove_objects]" value="{'Remove selected'|i18n( 'ezteamroom/edit' )}" />&nbsp;
        <input class="button" type="submit" name="CustomActionButton[{$attribute.id}_edit_objects]" value="{'Edit selected'|i18n( 'ezteamroom/edit' )}" />
        {section-else}
        <input class="button-disabled" type="submit" name="CustomActionButton[{$attribute.id}_remove_objects]" value="{'Remove selected'|i18n( 'ezteamroom/edit' )}" disabled="disabled" />&nbsp;
        <input class="button-disabled" type="submit" name="CustomActionButton[{$attribute.id}_edit_objects]" value="{'Edit selected'|i18n( 'ezteamroom/edit' )}" disabled="disabled" />
        {/section}

        {section show=array( 0, 2 )|contains( $class_content.type )}
        <input class="button" type="submit" name="CustomActionButton[{$attribute.id}_browse_objects]" value="{'Add objects'|i18n( 'ezteamroom/edit' )}" />
        ...
        {/section}
        {section-else}
        <input class="button-disabled" type="submit" name="CustomActionButton[{$attribute.id}_browse_objects]" value="{'Add objects'|i18n( 'ezteamroom/edit' )}" disabled="disabled" />
        {/section}
        ...
        <input class="button" type="submit" name="CustomActionButton[{$attribute.id}_new_class]" value="{'Create new object'|i18n( 'ezteamroom/edit' )}" />
        </div>
        {/section}
        ...
        <tr>
            ...
            <th>{'Type'|i18n( 'ezteamroom/edit' )}</th>
            <th>{'Section'|i18n( 'ezteamroom/edit' )}</th>
            <th>{'Published'|i18n( 'ezteamroom/edit' )}</th>
            <th class="tight">{'Order'|i18n( 'ezteamroom/edit' )}</th>
        </tr>
        ...
        <td>{if $Objects.item.in_trash|not() }
                {'Yes'|i18n( 'ezteamroom/edit' )}
                {else}
                {'No'|i18n( 'ezteamroom/edit' )}
            {/if}
        </td>
        {* Order. *}
