...
<tr>
    ...
    {section show=eq( $select_type, 'checkbox' )}
        <img src={'toggle-button-16x16.gif'|ezimage} alt="{'Invert selection'|i18n( 'ezteamroom/browse' )}" title="{'Invert selection'|i18n( 'ezteamroom/browse' )}" onclick="ezjs_toggleCheckboxes( document.browse, '{$select_name}[]' ); return false;" />
    {section-else}
        &nbsp;
    {/section}
    </th>
    <th class="wide">
    {'Name'|i18n( 'ezteamroom/browse' )}
    </th>
    <th class="tight">
    {'Type'|i18n( 'ezteamroom/browse' )}
    </th>
</tr>
     ...
     {section show=and( or( $browse.permission|not,
        ...
        {section show=is_array( $browse.class_array )}
            {section show=$browse.class_array|contains( $Nodes.item.object.content_class.identifier )}
                {* preselect inviteUserList*}
                {section show=$browse.inviteUserList|contains( $Nodes.item[$select_attribute] )}
                    <input type="{$select_type}" name="{$select_name}[]" value="{$Nodes.item[$select_attribute]}" checked />
                {* /preselect*}
                {section-else}
