...
{foreach $content_attributes as $attribute sequence array( 'bglight', 'bgdark' ) as $sequence}
    ...
    {if and(eq($attribute.contentclass_attribute.can_translate,0),
        ...
        <label class="title">{$attribute.contentclass_attribute.name|wash}{if $attribute.contentclass_attribute.is_required|eq('true')} *{/if}</label><div class="labelbreak"></div>
    ...
    {else}
        <label class="title{if $attribute.has_validation_error} validation-error{/if}">{$attribute.contentclass_attribute.name|wash}</label><div class="labelbreak"></div>
        <input type="hidden" name="ContentObjectAttribute_id[]" value="{$attribute.id}" />
