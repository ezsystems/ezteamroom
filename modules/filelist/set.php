...
$contentObject = eZContentObject::fetch( (int)$file );
...
if (  $contentObject )
{
    if ( $contentObject->attribute( 'can_edit' ) )
    {
        ...
        {
            eZDebug::writeError( "Unable to create new version of $file", 'filelist/set' );
            $error = 'kernel';
        }
        unset($contentObject);
    }
    ...
    {
        eZDebug::writeError( "User is not allowed to edit $file", 'filelist/set' );
        $error = 'access';
    }
}
else
{
    eZDebug::writeError( "Object does not exist $file", 'filelist/set' );
    $error = 'kernel';
}
...
else
{
    ...
    eZRedirectManager::redirectTo( $module, // $default
                                   false,   // $view
                                   true,    // $disallowed
                                   false,
                                   $redirectURI );
